...
def translate_text(source_text: str, source_lang: str, target_lang: str) -> str:
    ...
    messages = [
        {"role": "system", "content": "You are a professional translator. Maintain consistency with previous translations."}
    ]
...
class ContinuousAudioRecorder:
            ...
            def callback(indata, frames, time_info, status):
                ...
                with self.lock:
                    ...
                    if frames <= frames_to_end:
                        ...
                        self.buffer[self.buffer_index:self.buffer_index + frames] = indata
                        self.buffer_index = (self.buffer_index + frames) % self.buffer_size
                    else:
                        # We need to wrap around the buffer
                        self.buffer[self.buffer_index:] = indata[:frames_to_end]
                        self.buffer[0:frames - frames_to_end] = indata[frames_to_end:]
                    ...
                    try:
                        ...
                        if loop and not loop.is_closed():
                            ...
                            future.add_done_callback(
                                lambda f: logger.error(f"Signal error: {f.exception()}") 
    ...
    def _apply_noise_filter(self, audio_data):
        ...
        filtered_audio, self.hp_zi = signal.lfilter(
            self.hp_b, self.hp_a, audio_float.flatten(), zi=self.hp_zi
        )
        
        # Apply low-pass filter (remove high-frequency noise)
        filtered_audio, self.lp_zi = signal.lfilter(
            self.lp_b, self.lp_a, filtered_audio, zi=self.lp_zi
        )
...
def pad_audio_with_silence(audio_np: np.ndarray, current_sample_rate: int, 
                          current_duration_seconds: float, 
                          target_duration_seconds: float = 15.0) -> np.ndarray:
    """
        ...
        while True:
            try:
                ...
                async with asyncio.Semaphore(3):  # Limit to 3 concurrent audio processing tasks
                    ...
                    update_info = await continuous_recorder.wait_for_new_audio(
                        consumer_id=MAIN_CONSUMER_ID, 
                    ...
                    if remaining_time > 0:
                        ...
                        logger.warning(f"Processing took {elapsed_time:.2f}s, which exceeds the poll interval of {poll_interval}s. Starting next processing immediately.")
...
async def update_config(config: dict):
    ...
    # Handle audio settings changes while in local-audio-source mode
        ...
        if continuous_recorder is not None:
            ...
            needs_restart = (
                current_status['device_id'] != device_id or
                current_status['sample_rate'] != sample_rate or
            ...
            if needs_restart:
                ...
                settings_updated = continuous_recorder.update_settings(
                    sample_rate=sample_rate,
                    device_id=device_id,
                    buffer_seconds=buffer_seconds
                )
    ...
    if continuous_recorder:
        ...
        logger.info(f"Audio recorder status: running={status['running']}, "
...
async def control_audio_recorder(action: str):
    ...
    if action == "status":
        return {
            "action": action,
            "status": continuous_recorder.get_status() if continuous_recorder else None
        }
    ...
    if continuous_recorder is None:
        if action in ["stop", "restart"]:
            ...
            success = initialize_audio_recorder(
                sample_rate=sample_rate,
                device_id=device_id,
                buffer_seconds=buffer_seconds
            )
...
async def serve_index(request: Request, streaming: bool = False):
    ...
    context = {
        ...
        "languages": languages, # put value of all member of enum Language
    }
